...
### Phase E — リアルタイム複数人カーソル ✅ 完了
- **backend/websocket**: `cursor_move` を受信し、同社（`company_id`）の他ユーザーへブロードキャスト
- **cursor_leave**: disconnect 時に自動送信
- **useWebSocket**: `send()` メソッド追加（双方向通信対応）
- **RemoteCursors.tsx**: `useViewport()` で flow 座標→スクリーン変換。Figma 風カーソルSVG＋名前ラベル
- **スロットル**: 50ms 間引き送信（20fps）、8秒操作なしでフェードアウト
- コミット: `2583b68`
...
### Phase I — ピース詳細パネルのインライン編集 ✅ 完了
- タイトル・目的・評価指標・スキルタグをインライン編集（InlineTextEdit / InlineEditRow / InlineTagsEdit）
- 変更履歴ログタブ（piece_logs から取得）: イベント種別・変更前後diff・相対時刻表示
- コミット: 前セッション

### Phase J — プロジェクトテンプレートパネル ✅ 完了
- TemplatePanel.tsx: 右スライドパネル（400px）
- 既存プロジェクトを「保存」→ `POST /projects/:id/save-template`
- テンプレート一覧: タスク数・依存数・平均期間・先頭4タスクプレビュー
- 「このテンプレートを使う」→プロジェクト名入力→作成
- コミット: 前セッション

### Phase K — 組織ダッシュボードビュー ✅ 完了
- DashboardView.tsx: viewMode === 'dashboard' でオーバーレイ表示
- ヘルススコアリング（SVGリング）・KPIカード（オンタイム率・進行中・延滞・未割当・リスク）
- プロジェクト別進捗バー・リスクピース一覧・昨日の完了・人別/スキル別ベロシティ
- コミット: `a8b691f`
