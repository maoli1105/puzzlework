-- ============================================================
-- PuzzleWork デモシード: SaaS開発会社
-- 特徴: アジャイル開発 / 活発なスプリント / インフラ停滞 / 成熟リリース済みルーム
-- 実行: psql $DATABASE_URL -f scripts/seed_saas.sql
-- ============================================================
...
-- ▼ Sprint #12: 機能開発 (WARM ACTIVE ROOM)
-- フロントとバックが同時進行 → 2本のHEROピース + コリドーエッジ候補
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
  ...
  ('dd001001-0002-0000-0000-000000000000','フロントエンド実装',   'React + TypeScript でユーザー管理UI実装',       '11111111-1111-1111-1111-111111111111', ARRAY['frontend','react'], '2026-05-20','2026-06-03',4,'in_progress','cc001001-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='alice@saas-demo.jp'),  350000,  55,'2026-05-22', NULL),
  ('dd001001-0003-0000-0000-000000000000','バックエンド実装',     'Node.js + PostgreSQL でユーザーCRUD API実装',   '11111111-1111-1111-1111-111111111111', ARRAY['backend','api'],   '2026-05-20','2026-06-03',4,'in_progress','cc001001-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='bob@saas-demo.jp'),    350000,  70,'2026-05-21', NULL),
  ('dd001001-0004-0000-0000-000000000000','E2E テスト',           'フロント・バック統合テスト + シナリオ確認',      '11111111-1111-1111-1111-111111111111', ARRAY['qa','testing'],    '2026-06-03','2026-06-10',3,'ready',      'cc001001-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='carol@saas-demo.jp'),  150000,   0, NULL, NULL),
...
-- ▼ Sprint #9: バグ修正 (ACTIVE REPAIR ROOM)
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
  ('dd001002-0001-0000-0000-000000000000','バグ原因調査',         'ログ分析・スタックトレース解析・再現確認',       '11111111-1111-1111-1111-111111111111', ARRAY['debugging'],       '2026-05-24','2026-05-26',5,'done',       'cc001002-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='bob@saas-demo.jp'),    500000, 100,'2026-05-24','2026-05-26'),
  ('dd001002-0002-0000-0000-000000000000','修正実装 (API側)',     '認証トークン検証ロジックの修正・ユニットテスト',  '11111111-1111-1111-1111-111111111111', ARRAY['backend','fix'],   '2026-05-26','2026-05-30',5,'in_progress','cc001002-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='bob@saas-demo.jp'),    500000,  65,'2026-05-26', NULL),
  ('dd001002-0003-0000-0000-000000000000','修正実装 (フロント側)','エラーハンドリング改善・ユーザー通知UI修正',     '11111111-1111-1111-1111-111111111111', ARRAY['frontend','fix'],  '2026-05-26','2026-05-30',4,'in_progress','cc001002-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='alice@saas-demo.jp'),  300000,  45,'2026-05-27', NULL),
  ('dd001002-0004-0000-0000-000000000000','回帰テスト',           '修正後の全機能回帰テスト + 本番リグレッション確認','11111111-1111-1111-1111-111111111111', ARRAY['qa'],              '2026-05-30','2026-05-31',5,'ready',      'cc001002-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='carol@saas-demo.jp'),  500000,   0, NULL, NULL);

-- ▼ インフラ刷新: STALLED COOL ROOM (overdue pieces → cool room)
-- 計画は立てたが実装が止まっている / 期限超過
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
  ...
  ('dd001003-0002-0000-0000-000000000000','K8s移行計画策定',      'クラスター設計・移行手順書・ロールバック計画',    '11111111-1111-1111-1111-111111111111', ARRAY['infra','k8s'],     '2026-03-01','2026-04-01',3,'in_progress','cc001003-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='david@saas-demo.jp'),  200000,  15,'2026-03-10', NULL),
  ('dd001003-0003-0000-0000-000000000000','ステージング環境移行', 'K8sクラスターへのステージング環境移行と動作確認',  '11111111-1111-1111-1111-111111111111', ARRAY['devops','k8s'],    '2026-04-01','2026-05-01',3,'locked',     'cc001003-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='david@saas-demo.jp'),  180000,   0, NULL, NULL),
  ('dd001003-0004-0000-0000-000000000000','セキュリティ監査',     '外部セキュリティ診断・脆弱性対応',               '11111111-1111-1111-1111-111111111111', ARRAY['security'],        '2026-03-15','2026-04-30',4,'locked',     'cc001003-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='bob@saas-demo.jp'),    400000,   0, NULL, NULL),
  ('dd001003-0005-0000-0000-000000000000','本番環境移行',         '本番K8sクラスターへの完全移行',                  '11111111-1111-1111-1111-111111111111', ARRAY['devops','k8s'],    '2026-05-01','2026-05-15',3,'locked',     'cc001003-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='david@saas-demo.jp'),  300000,   0, NULL, NULL);

-- ▼ v2.3 リリース済み: MATURE ALL-DONE ROOM (完了2〜4ヶ月前)
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
...
-- ▼ ドキュメント整備: PRISTINE DORMANT ROOM
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
...
-- ▼ マーケティング連携: CROSS-TEAM (Sprint #12 へのコリドーエッジ生成用)
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
  ('dd001006-0001-0000-0000-000000000000','LP改善施策',           '新機能紹介LPのデザイン・コピー改善・A/Bテスト', '11111111-1111-1111-1111-111111111111', ARRAY['marketing','lp'],  '2026-05-15','2026-06-15',3,'in_progress','cc001006-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='emma@saas-demo.jp'),   180000, 40,'2026-05-20',NULL),
  ('dd001006-0002-0000-0000-000000000000','アナリティクス設定',   'GA4イベント設定・ファネル分析・コンバージョン計測','11111111-1111-1111-1111-111111111111', ARRAY['analytics'],       '2026-05-20','2026-06-10',3,'in_progress','cc001006-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='emma@saas-demo.jp'),   150000, 30,'2026-05-24',NULL),
...
INSERT INTO connections (from_piece_id, to_piece_id, type) VALUES
  ...
  ('dd001001-0002-0000-0000-000000000000','dd001001-0004-0000-0000-000000000000','sequential'),
  ('dd001001-0003-0000-0000-000000000000','dd001001-0004-0000-0000-000000000000','sequential'),
  ('dd001001-0004-0000-0000-000000000000','dd001001-0005-0000-0000-000000000000','sequential')
...
INSERT INTO connections (from_piece_id, to_piece_id, type) VALUES
  ...
  ('dd001002-0002-0000-0000-000000000000','dd001002-0004-0000-0000-000000000000','sequential'),
  ('dd001002-0003-0000-0000-000000000000','dd001002-0004-0000-0000-000000000000','sequential')
ON CONFLICT DO NOTHING;

-- インフラ刷新
INSERT INTO connections (from_piece_id, to_piece_id, type) VALUES
  ('dd001003-0001-0000-0000-000000000000','dd001003-0002-0000-0000-000000000000','sequential'),
  ('dd001003-0002-0000-0000-000000000000','dd001003-0003-0000-0000-000000000000','sequential'),
  ('dd001003-0002-0000-0000-000000000000','dd001003-0004-0000-0000-000000000000','sequential'),
  ('dd001003-0003-0000-0000-000000000000','dd001003-0005-0000-0000-000000000000','sequential')
ON CONFLICT DO NOTHING;
...
-- ── クロスプロジェクト コリドーエッジ ─────────────────────────
-- Sprint #12 のリリース準備 ← マーケ LP改善 (リリースに合わせて発動)
-- Sprint #12 のリリース準備 ← マーケ アナリティクス設定 (計測準備)
-- → corridorDensity['cc001001|cc001006'] = 2 → isCorridorEdge = true
INSERT INTO connections (from_piece_id, to_piece_id, type) VALUES
  ('dd001006-0001-0000-0000-000000000000','dd001001-0005-0000-0000-000000000000','parallel'),
  ('dd001006-0002-0000-0000-000000000000','dd001001-0005-0000-0000-000000000000','parallel')
ON CONFLICT DO NOTHING;
