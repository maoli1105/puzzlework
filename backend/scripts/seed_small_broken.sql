-- ============================================================
-- PuzzleWork デモシード: 小規模チーム（5人）— 修復前（破損状態）
-- 特徴: X案件も含め全員over-committedで停滞 / Yは完全放置 / コリドー断線
-- ============================================================
...
-- ▼ クライアントX: ヒアリングで止まっている (要件固まらず着手できない)
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
  ('dd005001-0001-0000-0000-000000000000','追加機能 要件ヒアリング',  'クライアントとのMTG・追加開発スコープ確定',                '11111111-1111-1111-1111-111111111111',ARRAY['consulting','spec'],   '2026-04-20','2026-05-05',5,'in_progress','cc005001-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='owner@small-demo.jp'),    500000,  8,'2026-04-20',NULL),
  ('dd005001-0002-0000-0000-000000000000','UIデザイン変更',           '新機能追加に伴うUI改善・モック作成',                        '11111111-1111-1111-1111-111111111111',ARRAY['design','ui'],         '2026-05-01','2026-05-15',5,'locked','cc005001-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='nakata@small-demo.jp'),   300000,  0,NULL,NULL),
  ('dd005001-0003-0000-0000-000000000000','バックエンド実装',         'DB設計変更・API追加・既存機能改修',                         '11111111-1111-1111-1111-111111111111',ARRAY['backend','api'],       '2026-05-01','2026-05-15',5,'locked','cc005001-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='saito@small-demo.jp'),    500000,  0,NULL,NULL),
  ('dd005001-0004-0000-0000-000000000000','フロントエンド実装',       '新UIをNext.jsで実装・既存画面との統合',                     '11111111-1111-1111-1111-111111111111',ARRAY['frontend','react'],    '2026-06-07','2026-06-15',5,'locked','cc005001-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='hashimoto@small-demo.jp'), 400000,  0,NULL,NULL),
  ('dd005001-0005-0000-0000-000000000000','テスト・受入確認',         'バグ修正・クライアントレビュー・承認取得',                  '11111111-1111-1111-1111-111111111111',ARRAY['qa','cs'],             '2026-06-20','2026-06-25',5,'locked','cc005001-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='nakata@small-demo.jp'),   200000,  0,NULL,NULL),
...
-- ▼ クライアントY: 3ヶ月放置 / 問題が積み上がっている
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
  ('dd005002-0001-0000-0000-000000000000','月次バグ修正 (3月分)',    '3月報告分のバグ修正 (ずっと後回し)',                        '11111111-1111-1111-1111-111111111111',ARRAY['maintenance','fix'],   '2026-03-01','2026-03-31',2,'in_progress','cc005002-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='saito@small-demo.jp'),     80000,  2,'2026-03-01',NULL),
  ('dd005002-0002-0000-0000-000000000000','月次バグ修正 (4月分)',    '4月報告分のバグ修正 (まだ着手できていません)',              '11111111-1111-1111-1111-111111111111',ARRAY['maintenance','fix'],   '2026-04-01','2026-04-30',2,'locked','cc005002-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='saito@small-demo.jp'),     80000,  0,NULL,NULL),
  ('dd005002-0003-0000-0000-000000000000','サーバー更新対応',        'Node.jsバージョンアップ・依存パッケージ更新',               '11111111-1111-1111-1111-111111111111',ARRAY['maintenance','devops'], '2026-04-01','2026-04-20',3,'locked','cc005002-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='hashimoto@small-demo.jp'),  60000,  0,NULL,NULL);
...
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
  ...
  ('dd005003-0002-0000-0000-000000000000','MVP設計 (ワイヤーフレーム)','主要機能3つのUI/UX設計',                                 '11111111-1111-1111-1111-111111111111',ARRAY['design','ux'],         '2026-03-01','2026-04-15',1,'in_progress','cc005003-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='nakata@small-demo.jp'),    30000,  2,'2026-03-01',NULL),
...
-- ▼ クライアントZ: ヒアリングすら止まっている
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
  ('dd005004-0001-0000-0000-000000000000','キックオフ・ヒアリング',  '新規クライアント オンボーディング・要件ヒアリング',         '11111111-1111-1111-1111-111111111111',ARRAY['consulting'],          '2026-05-01','2026-05-12',4,'in_progress','cc005004-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='owner@small-demo.jp'),    200000,  5,'2026-05-01',NULL),
  ('dd005004-0002-0000-0000-000000000000','提案書・見積もり作成',    '技術提案・スケジュール・費用見積もり',                      '11111111-1111-1111-1111-111111111111',ARRAY['proposal','planning'], '2026-05-12','2026-05-20',4,'locked','cc005004-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='owner@small-demo.jp'),    400000,  0,NULL,NULL),
  ('dd005004-0003-0000-0000-000000000000','設計・開発着手',          '提案承認後 詳細設計開始',                                   '11111111-1111-1111-1111-111111111111',ARRAY['design','dev'],        '2026-06-01','2026-06-10',4,'locked','cc005004-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='hashimoto@small-demo.jp'), 600000,  0,NULL,NULL);
