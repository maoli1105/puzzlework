-- ============================================================
-- PuzzleWork デモシード: Web制作会社 — 修復前（破損状態）
-- 特徴: クライアントB完全停滞 / LP進行断絶 / コリドー切断 / 全案件冷却
-- ============================================================
...
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
  ...
  ('dd002001-0003-0000-0000-000000000000','デザイン制作',          'トップ・会社概要・製品・採用 デザイン4画面',             '11111111-1111-1111-1111-111111111111',ARRAY['design','ui'],        '2026-05-01','2026-05-18',4,'in_progress','cc002001-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='sato@webagency-demo.jp'), 400000, 12,'2026-05-01',NULL),
  ('dd002001-0004-0000-0000-000000000000','HTML/CSSコーディング',  'レスポンシブ対応・アクセシビリティ・CMS組み込み',        '11111111-1111-1111-1111-111111111111',ARRAY['html','css','coding'], '2026-06-02','2026-06-10',4,'locked','cc002001-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='tanaka@webagency-demo.jp'),  500000,  0,NULL,NULL),
...
-- ▼ クライアントB: デザインが長期停滞 → 全下流がblocked (完全凍結)
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
  ...
  ('dd002002-0002-0000-0000-000000000000','デザイン (商品ページ)', 'トップ・カテゴリー・商品詳細・カート画面デザイン',        '11111111-1111-1111-1111-111111111111',ARRAY['design','ec'],        '2026-03-10','2026-03-28',4,'in_progress','cc002002-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='sato@webagency-demo.jp'),  600000,  8,'2026-03-10',NULL),
  ('dd002002-0003-0000-0000-000000000000','Shopify実装',          'テーマカスタマイズ・商品データ移行・決済設定',            '11111111-1111-1111-1111-111111111111',ARRAY['shopify','coding'],   '2026-04-11','2026-04-25',5,'locked','cc002002-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='tanaka@webagency-demo.jp'),  800000,  0,NULL,NULL),
  ('dd002002-0004-0000-0000-000000000000','決済・配送設定',        '決済ゲートウェイ・送料計算・在庫管理連携',               '11111111-1111-1111-1111-111111111111',ARRAY['payment','integration'],'2026-04-30','2026-05-10',5,'locked','cc002002-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='tanaka@webagency-demo.jp'),  400000,  0,NULL,NULL),
  ('dd002002-0005-0000-0000-000000000000','テスト・公開',          '注文フロー確認・クロスブラウザ・本番公開',               '11111111-1111-1111-1111-111111111111',ARRAY['qa','deploy'],        '2026-05-15','2026-05-20',5,'locked','cc002002-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='suzuki@webagency-demo.jp'),  200000,  0,NULL,NULL);

-- ▼ クライアントC LP: LP-1完了後LP-2/3が止まっている / 分断状態
INSERT INTO pieces (id, title, objective, company_id, skill_tags, start_date, due_date, priority, status, project_id, assignee_id, business_impact, progress, started_at, completed_at) VALUES
  ...
  ('dd002003-0002-0000-0000-000000000000','LP-1 コーディング',      'SP対応・GTM設定・フォーム実装',                         '11111111-1111-1111-1111-111111111111',ARRAY['lp','coding'],        '2026-05-20','2026-05-28',3,'locked','cc002003-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='tanaka@webagency-demo.jp'),  180000,  0,NULL,NULL),
  ('dd002003-0003-0000-0000-000000000000','LP-2 デザイン',          'サプリ新商品LP デザイン',                               '11111111-1111-1111-1111-111111111111',ARRAY['lp','design'],        '2026-05-01','2026-05-15',3,'in_progress','cc002003-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='sato@webagency-demo.jp'), 150000,  6,'2026-05-01',NULL),
  ...
  ('dd002003-0005-0000-0000-000000000000','LP-3 デザイン',          'フード新商品LP デザイン',                               '11111111-1111-1111-1111-111111111111',ARRAY['lp','design'],        '2026-06-01','2026-06-06',2,'locked','cc002003-0000-0000-0000-000000000000',(SELECT id FROM users WHERE email='sato@webagency-demo.jp'),    120000,  0,NULL,NULL),
